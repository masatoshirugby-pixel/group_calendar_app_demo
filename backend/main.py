...
async def manual_fetch(
    ...
    end_time: str = Query(default=None, description="[テスト用] 取得終了日時 (RFC3339例: 2026-04-08T13:00:00Z)"),  # テスト用（通常運用では不要）
    max_results: int = Query(default=100, ge=5, le=100, description="1アカウントあたりの最大取得件数"),
):
    """手動トリガー: Web + X のフルパイプラインを全グループ実行"""
    try:
        count = await asyncio.to_thread(scheduler.run_pipeline, start_time, end_time, max_results)
